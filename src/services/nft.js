...
export const mintWarrantyNFT2 = async (creatorPubkey, { name, description, warrantyPeriod }) => {
  const metadata = {
    name,
...
export async function mintWarrantyNFT(
  wallet,
  metadata
){
  ...
  const detailedNfts = await Promise.all(
    nfts.map(async (nft) => {
      try {
        ...
        console.warn(`Помилка при завантаженні метаданих для ${nft.uri}:`, error);
        return {
          ...nft,
          json: null,
        };
